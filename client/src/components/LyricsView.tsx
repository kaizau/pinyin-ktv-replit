...
export default function LyricsView({
  ...
  useEffect(() => {
    ...
    const processLyrics = async () => {
      ...
      if (data.syncedLyrics) {
        ...
        const syncedLines = data.syncedLyrics
          .split("\n")
        ...
        for (let i = 0; i < syncedLines.length; i++) {
          ...
          if (timeMatch) {
            ...
            if (i < syncedLines.length - 1) {
              const nextTimeMatch = syncedLines[i + 1].match(
                /^\[(\d{2}):(\d{2})\.(\d{2})\]/,
              );
              if (nextTimeMatch) {
  ...
  if (
    !data ||
    (!data.plainLyrics && !data.syncedLyrics && !data.instrumental)
  ) {
    return (
      <div className="h-full flex items-center justify-center p-4">
    ...
    <div className="h-full overflow-hidden flex flex-col">
      ...
      >
        {syncedLyrics.map((line, index) => (
          <div
            key={index}
            className={`mb-2 p-2 rounded transition-colors duration-300 flex justify-between ${
          ...
          >
            ...
            {hasSyncedLyrics && (
              <div className="text-xs text-text-muted self-center whitespace-nowrap ml-2">
                ...
                {(line.startTime % 60).toFixed(0).padStart(2, "0")}
              </div>
            )}
          </div>
        ))}
